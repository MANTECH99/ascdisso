...
<div class="grid grid-cols-3 gap-4 mb-8">
    <div class="bg-white rounded-lg shadow p-4 text-center">
        ...
        <p class="text-xl font-bold" style="color: #4D1111">{{ $logs->whereIn('service_code', ['OM_SN_CASHIN', 'WAVE_SN_CASHIN', 'FM_SN_CASHIN', 'WIZALL_SN_CASHIN'])->count() }}</p>
        <p class="text-xs text-gray-400">Total</p>
    </div>
    <div class="bg-white rounded-lg shadow p-4 text-center">
        <p class="text-xs text-gray-500 mb-1">Réceptions</p>
        <p class="text-xl font-bold" style="color: #10B981">{{ $logs->whereIn('service_code', ['OM_SN_CASHOUT', 'WAVE_SN_CASHOUT', 'FM_SN_CASHOUT', 'WIZALL_SN_CASHOUT'])->count() }}</p>
        <p class="text-xs text-gray-400">Total</p>
    </div>
    <div class="bg-white rounded-lg shadow p-4 text-center">
            ...
            <form action="{{ route('admin.cashout.initiate') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="service" required
                        ...
                        <option value="OM_SN_CASHIN">🟠 Orange Money (1.5%)</option>
                        <option value="WAVE_SN_CASHIN">🔵 Wave (1.5%)</option>
                        <option value="FM_SN_CASHIN">🟢 Free Money (1.5%)</option>
                        <option value="WIZALL_SN_CASHIN">🟣 Wizall (1.5%)</option>
                    </select>
                    @error('service')
                ...
                <div class="mb-6">
                    ...
                    <div class="relative">
                        <input type="number" name="amount" value="{{ old('amount') }}" required min="100" step="1"
                               placeholder="5000"
                               class="w-full border border-gray-300 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500">
                        <span class="absolute right-3 top-2.5 text-gray-400 font-medium">FCFA</span>
                    </div>
                    <p class="text-xs text-gray-400 mt-1">Minimum : 100 FCFA</p>
        ...
        <span class="w-8 h-8 rounded-full flex items-center justify-center mr-2 text-white text-sm"
              style="background-color: #4D1111">📋</span>
                ...
                @php
                    $envois = $logs->whereIn('service_code', ['OM_SN_CASHIN', 'WAVE_SN_CASHIN', 'FM_SN_CASHIN', 'WIZALL_SN_CASHIN']);
                    $receptions = $logs->whereIn('service_code', ['OM_SN_CASHOUT', 'WAVE_SN_CASHOUT', 'FM_SN_CASHOUT', 'WIZALL_SN_CASHOUT']);
                @endphp
