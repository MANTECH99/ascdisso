...
<div class="flex justify-between items-center py-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 px-2 rounded"
    ...
    <div>
        <p class="font-medium text-sm">{{ $isCashin ? 'À' : 'De' }} &nbsp;{{ substr($log->phone, 0, 2) }} {{ substr($log->phone, 2, 3) }} {{ substr($log->phone, 5, 2) }} {{ substr($log->phone, 7, 2) }}</p>
    ...
    <div class="p-6">
        <div class="text-center mb-4">
            @if($isCashin)
                ...
                <p class="text-gray-500 mt-2">À {{ substr($log->phone, 0, 2) }} {{ substr($log->phone, 2, 3) }} {{ substr($log->phone, 5, 2) }} {{ substr($log->phone, 7, 2) }}</p>
            @else
                <p class="text-4xl font-black" style="color: #10B981">+{{ number_format($montantRecu, 0) }} FCFA</p>
                <p class="text-gray-500 mt-2">De {{ substr($log->phone, 0, 2) }} {{ substr($log->phone, 2, 3) }} {{ substr($log->phone, 5, 2) }} {{ substr($log->phone, 7, 2) }}</p>
            @endif
        </div>
